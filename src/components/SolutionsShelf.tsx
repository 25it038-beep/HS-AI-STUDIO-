...
const SOLUTIONS = [
  {
    ...
    neonColor: "#a855f7",
    glowClass: "shadow-[0_0_30px_rgba(168,85,247,0.2)] hover:border-purple-400/60",
  ...
  },
  {
    ...
    neonColor: "#06b6d4",
    glowClass: "shadow-[0_0_30px_rgba(6,182,212,0.2)] hover:border-cyan-400/60",
  ...
  },
  {
    ...
    neonColor: "#10b981",
    glowClass: "shadow-[0_0_30px_rgba(16,185,129,0.2)] hover:border-emerald-400/60",
  ...
  },
  {
    ...
    neonColor: "#f59e0b",
    glowClass: "shadow-[0_0_30px_rgba(245,158,11,0.25)] hover:border-amber-400/60",
...
export default function SolutionsShelf({ onDownload }: { onDownload?: () => void }) {
  return (
    <section id="solutions" className="relative z-10 py-28 md:py-36 px-4 sm:px-6 lg:px-8 cyber-dots">
...
function BentoItem({
  ...
  return (
    ...
    >
      <div className={`bento-card p-7 sm:p-8 h-full flex flex-col justify-between relative overflow-hidden group ${solution.glowClass}`}>
        ...
        <div className="relative z-10 flex flex-col h-full justify-between gap-6">
          ...
          <div className="flex items-center justify-between gap-3 h-12">
            <div className="w-12 h-12 rounded-2xl bg-white/[0.05] border border-white/10 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">
              {solution.icon}
            </div>

            <div className={`px-3 py-1 rounded-full text-[10px] font-mono font-bold uppercase tracking-wider border ${solution.badgeBg}`}>
          ...
          <div className="min-h-[60px] flex flex-col justify-center gap-1">
            ...
            <p className="text-xs font-mono font-semibold uppercase text-zinc-400 tracking-wider">
              {solution.tagline}
            </p>
          </div>
          ...
          <div className="space-y-2 pt-2">
            {solution.features.map((feat) => (
              <div key={feat} className="flex items-center gap-2 text-[11px] text-zinc-300">
                <span className="w-1.5 h-1.5 rounded-full" style={{ background: solution.neonColor }} />
                <span>{feat}</span>
              </div>
            ))}
          </div>
            ...
            {isDev ? (
              <>
                <a
                  ...
                  download
                  onClick={onDownload}
                  className={`w-full h-12 inline-flex items-center justify-center gap-2 px-5 rounded-2xl ${solution.btnClass} text-xs uppercase tracking-wider font-extrabold shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all`}
                >
